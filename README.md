...
```
Set a passphrase to protect access to your key, using `ssh-add` to remember
...
The `inventory/group_vars/all/secrets.yml` file specifies deploy secrets.
...
This playbook sets the `$HOME/.erlang.cookie` file on the web server:
